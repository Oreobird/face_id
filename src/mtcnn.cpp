...
void MTCNN::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox> &boundingBox_, float scale) {
    ...
    for (int row = 0; row < score.h; row++) {
        ...
        int line = row * score.w;
        float *location_line[4] = {
                location.channel(0) + line,
                location.channel(1) + line,
                location.channel(2) + line,
                location.channel(3) + line,
        };
        for (int col = 0; col < score.w; col++) {
            if (*p > thres) {
                ...
                bbox.area = (bbox.x2 - bbox.x1) * diff_y;
                for (int channel = 0; channel < 4; channel++) {
                    bbox.regreCoord[channel] = location_line[channel][col];
                }
...
void MTCNN::refine(vector<Bbox> &vecBbox, const int &height, const int &width, bool square) {
    ...
    for (vector<Bbox>::iterator it = vecBbox.begin(); it != vecBbox.end(); it++) {
        ...
        if (square) {
            ...
            y1 = y1 + h * 0.5f - maxSide * 0.5f  + 0.02f * maxSide;
        ...
        if (square) {
	        ...
	        h = y2 - y1 + 1;
	        midSide = (h + w) / 2;
	        x1 = x1 + w * 0.5f - midSide * 0.5f;
	        y1 = y1 + h * 0.5f - midSide * 0.5f + 0.05f * midSide;
	        (*it).x2 = (int) (x1 + midSide - 1 + 0.5f);
	        (*it).y2 = (int) (y1 + midSide - 1 + 0.5f);
	        (*it).x1 = (int) (x1 + 0.5f);
	        (*it).y1 = (int) (y1 + 0.5f);
        }
        #endif
...
void MTCNN::detect_face(cv::Mat &img_, std::vector<face_box_t> &faces)
{
    int width = img_.cols;
    int height = img_.rows;

    ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(img_.data, ncnn::Mat::PIXEL_BGR2RGB, width, height);
    ...
    //dbg("firstBbox_.size()=%ld\n", firstBbox_.size());


    //second stage
    RNet();
    //dbg("secondBbox_.size()=%ld\n", secondBbox_.size());
	...
	for (std::vector<Bbox>::iterator iter = thirdBbox_.begin(); iter != thirdBbox_.end(); ++iter)
	{
		face_box_t face;
		face.box = (*iter);
		face.name = "unknown";
