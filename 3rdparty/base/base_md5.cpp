...
//MD5字符串摘要
std::string md5(const char * str, unsigned int size)
{
	std::string result = "";
	...
	*(x++) = size>>29;
	...
	DELETE_POINTER_ARR(strw);
	
	char md5[50] = {0};
	snprintf(md5, 49, "%08x%08x%08x%08x", conv(val.a), conv(val.b), conv(val.c), conv(val.d));	
	result = md5;
